...
public class JanelaCompra extends JFrame {
    ...
    // TODO FEITO criar uma nova venda assim que se inicia
    private Venda vendaAtual = new Venda(LocalDate.now(), new ArrayList<>(), new ArrayList<>());
    ...
    private void adicionarProdutoVenda(ProdutoInfo p) {
        // TODO FEITO adicionar o produto vendido à venda
        ProdutoVendido produtoVendido = new ProdutoVendido(p, p.getPrecoAtual(), 0);
        vendaAtual.adicionarProduto(produtoVendido);
        // TODO FEITO atualizar o total
        long total = vendaAtual.getTotal();
    ...
    private void pagarComCartao(Cartao c) {
        // TODO FEITO ver se o cartão esta ativo antes de proceder ao pagamento
        if (!c.estaAtivo()) {
            JOptionPane.showMessageDialog(this, "Por favor, ative cartão na aplicação!");
            return;
        }

        // TODO FEITO colocar a informação certa nas variáveis feito
        long saldoCartao = c.getSaldo();
        
        long totalVenda = vendaAtual.getTotal();

        if (saldoCartao > 0) {
            int opcao = JOptionPane.showConfirmDialog(this,
                    "Deseja usar o saldo de " + precoToString(saldoCartao) + "?", "Usar saldo",
                    JOptionPane.YES_NO_OPTION);
            // TODO FEITO se quer usar o saldo é preciso gastá-lo feito
            if (opcao == JOptionPane.YES_OPTION) {
                c.reduzirSaldo(totalVenda);
            }
        }

        // TODO FEITO usar o cartão na compra
        c.usar(vendaAtual);
        int numeroCupoesUsados = vendaAtual.getCupoesAplicados().size();
        ...
        // TODO FEITO criar uma nova venda
        vendaAtual = new Venda(LocalDate.now(), new ArrayList<>(), new ArrayList<>());
    ...
    private long getPrecoProduto(ProdutoInfo p) {
        // TODO FEITO retornar o valor certo feito feito
    ...
    private String getMarcaProduto(ProdutoInfo p) {
        // TODO FEITO substituir o texto pelo valor certo feito
        return p.getMarca();
    }
    ...
    private String getModeloProduto(ProdutoInfo p) {
        // TODO substituir o texto pelo valor certo feito
        return p.getModelo();
    }
    ...
    private final class RendererCartoes extends DefaultListCellRenderer {
        ...
        public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected,
            ...
            // TODO FEITO colocar a informação certa na variável
    ...
    private void setupJanela(Inventario inv) {
        ...
        setResizable(false);
        // TODO FEITO passar a coleção de produtos (java.util.List.of() só está a criar uma
        // lista vazia)
        JPanel inventario = setupInventario(inv.getProdutos());
        // TODO FEITO passar a coleção de cartões (java.util.List.of() só está a criar uma
        // lista vazia)
